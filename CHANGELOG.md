...
> This file was generated by release-please, which stopped running after 0.17.0
...
* **piscem >= 0.22.0 and alevin-fry >= 0.17.0 are now hard requirements.**
  `simpleaf set-paths` rejects older binaries instead of warning. The three
  previously disagreeing version floors (`>=0.19.0`, `>=0.18.0`, `>=0.17.1`)
  are now defined once in `prog_utils::min_versions`. Both versions are
  available on bioconda.
* **`-t`/`--threads` now covers gzip decompression as well as mapping**, since
  piscem 0.22.0 treats it as a single shared budget. A given `-t` may map at a
  different rate than it did before.
* **`--ram-limit-gib` defaults to 8** rather than deferring to piscem, whose own
  default scales with the host's total RAM. This restores the pre-0.22.0 index
  build footprint (the same human splici index peaked at 22 GB vs 42 GB on a
  larger machine) at no measurable cost in wall time.

### Features

* `--decoder` and `--thread-policy` are forwarded to piscem from `quant`,
...
* `gene_id_to_name.tsv` emitted from probe sets that carry a gene-name column,
  and used to populate `var["gene_symbol"]` (#202).
* Documentation moved to https://combine-lab.github.io/simpleaf.
...
* A non-semver `--version` string from a dependency panicked instead of
  producing an error.
* The macs3 executable was looked up through the `ALEVIN_FRY` environment
  variable.
* The thread-cap warning now explains that the limit reflects CPU affinity or a
  cgroup rather than the machine's core count (#135).
