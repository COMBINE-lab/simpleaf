...
Direct Reference Options:
      ...
      --excluded-probes <EXCLUDED_PROBES>
          How probes flagged `included = FALSE` in `--probe-csv` are treated. `decoy` (default)
          indexes their sequences as piscem decoys (alongside any `--decoy-paths`) so reads from an
          excluded probe are recognised and discarded rather than mis-assigned to a retained probe;
          `ignore` drops them from the reference entirely. Excluded probes are never quantified in
          either mode. Has no effect on other reference types
